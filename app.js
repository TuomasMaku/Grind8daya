...
/* ── NEW STATE: charges, notes, smart rest ── */
let chargesData  = store.get('chargesData', {});    // {exName: [{date,kg,reps},...]}
let sessionNotes = store.get('sessionNotes', {});   // {d0, d1, ...}: string
let dayConfirms  = store.get('dayConfirms', {});    // {YYYY-MM-DD: 'done'|'skipped'}
...
function saveWeight(){
  ...
  // Remove existing entry for today if any, then push new
...
function confirmDay(status){
  ...
  if(status === 'skipped'){
    if(hardcoreMode){
      ...
      cycleOffset++;
      store.set('cycleOffset', cycleOffset);
      refreshCurDay();
    }
  }

  if(srCard){
    srCard.style.opacity='0';
    srCard.style.transform='translateY(-8px)';
...
function updateWeekdays(){
  if(!cycleStart){
    days.forEach((day,i)=>{
      ...
      if(el) el.textContent = day.weekday;
    });
    return;
  }
  const start = new Date(cycleStart);
...
document.getElementById('resetProg').addEventListener('click',()=>{
  ...
  cycleOffset=0;
  store.set('progState',progState);
  store.set('cycleStart',cycleStart);
  store.set('cycleOffset',cycleOffset);
...
challenges.forEach((ch,i)=>{
  ...
  minus.className='ch-btn minus'; minus.textContent='−'+ch.step;
  const plus=document.createElement('button');
  plus.className='ch-btn primary'; plus.textContent='+'+ch.step;

  function renderCh(){
    const v=dailyState.reps[i];
    const p=Math.min(v/ch.target*100,100);
...
function renderStats(){
  ...
  if(heatEl){
    ...
    const histMap = {};
    historyData.forEach(e=>{ histMap[e.date]=e; });
    ...
    while(cur <= today){
      ...
      for(let d=0;d<7;d++){
        ...
        if(cur > today){
          ...
          if(entry) cell.className='hm-cell '+(entry.done?'done':'partial');
          else cell.className='hm-cell';
        }
  ...
  const exTotals = challenges.map((_,i)=>
    historyData.reduce((a,d)=>a+(((d.reps||[])[i])||0),0)
  );
  const maxEx = Math.max(...exTotals, 1);
  const exEl = document.getElementById('exTotals');
  exEl.innerHTML = '';
  challenges.forEach((ch,i)=>{
    const pct = Math.round(exTotals[i]/maxEx*100);
    const row = document.createElement('div');
    row.className = 'ex-total-row';
    row.innerHTML = `
      <div class="ex-total-icon">${ch.icon}</div>
      <div class="ex-total-body">
        <div class="ex-total-name">${ch.name}</div>
        <div class="ex-total-bar-wrap">
          <div class="ex-total-bar" style="width:${pct}%;background:${CH_COLORS[i]}"></div>
        </div>
      </div>
  ...
  // Progression charges (nouveau section)
  renderChargesStats();
  ...
  historyData.slice(0,60).forEach((entry,idx)=>{
    ...
    row.innerHTML = `
      <div class="hist-date-block">
        ...
        <div class="hist-month">${mois[d.getMonth()]}</div>
      </div>
      <div class="hist-body">
        <div class="hist-exercises">${chips||'<span style="color:var(--muted);font-size:12px">—</span>'}</div>
        <div class="hist-bar-wrap"><div class="hist-bar" style="width:${entry.pct}%"></div></div>
      </div>
      <div class="hist-right">
        <div class="hist-pct">${entry.pct}%</div>
        <div class="hist-badge">${entry.done?'✓ Complet':'En cours'}</div>
      </div>`;
...
function renderChargesStats(){
  const el = document.getElementById('chargesStats');
  if(!el) return;
  el.innerHTML='';

  const exNames = Object.keys(chargesData).filter(n=>chargesData[n].length>0);
  if(!exNames.length){
    el.innerHTML='<div class="history-empty">Aucune charge enregistrée.<br>Tape sur le badge kg dans une séance 💪</div>';
    return;
  }

  // Key lifts first (known exercises)
  ...
  ordered.forEach(name=>{
    ...
    row.innerHTML=`
      ...
      <div class="charges-right">
        <div class="charges-max">${last.kg}<span class="charges-unit">kg</span></div>
        <div class="charges-reps">× ${last.reps}${deltaStr}</div>
      </div>`;
    el.appendChild(row);
  });
}
...
function updateHome(){
  ...
  setTxt('hardcoreSub', hardcoreMode
    ? '🔴 ACTIF — Skip ou daily incomplet = streak reset'
    : 'Skip = streak reset. Zéro pitié.'
  );
  setTxt('streakNum', s);
...
document.getElementById('modalSkipCancel').addEventListener('click',()=>{
...
document.getElementById('startCycleBtn').addEventListener('click',()=>{
  if(cycleStart){
    document.getElementById('modalCycleOverlay').classList.add('show');
  } else {
    doStartCycle();
  }
});

document.getElementById('modalCycleCancel').addEventListener('click',()=>{
  document.getElementById('modalCycleOverlay').classList.remove('show');
});
document.getElementById('modalCycleConfirm').addEventListener('click',()=>{
  document.getElementById('modalCycleOverlay').classList.remove('show');
  doStartCycle();
});

document.getElementById('resetStats').addEventListener('click',()=>{
  document.getElementById('modalStatsOverlay').classList.add('show');
});
document.getElementById('modalStatsCancel').addEventListener('click',()=>{
...
/* ── BACKUP / RESTORE ── */
function exportData(){
  const data = { progState, cycleStart, cycleOffset, dailyState, streakData, historyData, chargesData, sessionNotes, dayConfirms };
  ...
  reader.onload = function(e){
    try {
      ...
      if(data.streakData && typeof data.streakData==='object') store.set('streakData', {
        count: Number(data.streakData.count||0),
        lastDate: data.streakData.lastDate||'',
        best: Number(data.streakData.best||0)
      });
...
function checkLevelUp(){
  ...
  if(!rankState.lastRank){
    rankState.lastRank = currentRank.name;
    store.set('rankState', rankState);
    return;
  }
...
function toggleHardcore(){
  ...
  store.set('hardcoreMode', hardcoreMode);
  const toggle = document.getElementById('hardcoreToggle');
  if(toggle) toggle.classList.toggle('on', hardcoreMode);
  setTxt('hardcoreSub', hardcoreMode
    ? '🔴 ACTIF — Skip ou daily incomplet = streak reset'
...
function triggerHardcoreFail(){
  ...
  setTxt('hcTitle', msg.title);
  setTxt('hcSub', msg.sub);
  streakData.count = 0;
  store.set('streakData', streakData);
...
// Smart rest: store last open date for next visit
